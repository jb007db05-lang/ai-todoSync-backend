...
class TaskService {
  public async createTask(payload: CreateTaskPayload): Promise<TaskDto> {
    ...
    if (payload.projectId) {
      await projectService.assertProjectOwnership(payload.userId, payload.projectId);
    }
    ...
    payload.subtasks = this.applyCompletedStatusToSubtasks(payload.status, payload.subtasks);
  ...
  public async updateTask(taskId: string, userId: string, updates: UpdateTaskPayload): Promise<TaskDto> {
    if (Object.prototype.hasOwnProperty.call(updates, 'projectId') && updates.projectId) {
      await projectService.assertProjectOwnership(userId, updates.projectId);
    }

    if (Object.prototype.hasOwnProperty.call(updates, 'status') && updates.status != null) {
  ...
  public async getSummary(userId: string, date?: unknown): Promise<TaskSummary> {
    ...
    return { total, pending, inProgress, inReview, completed, rolledOver, date: normalizedDate };
  }
  ...
  private normalizeSubtasks(subtasks: unknown): ISubtask[] {
    ...
    return subtasks.map((subtask, index) => this.normalizeSubtask(subtask, index));
  }
  ...
  private normalizeSubtask(value: unknown, index: number): ISubtask {
    ...
    const completedAt = completed ? this.normalizeCompletedAt(subtask.completedAt) : null;
