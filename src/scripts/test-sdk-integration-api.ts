...
async function login() {
  ...
  if (!token) throw new Error(`Login failed. Response: ${JSON.stringify(data)}`);
  authToken = token;
}
...
async function testSdkIntegrationCRUD() {
  ...
  await runTest("POST /sdk-integrations — creates integration, returns raw SDK key", async () => {
  ...
  await runTest("POST /sdk-integrations — rejects missing name (400)", async () => {
    const r = await authedFetch("POST", "/sdk-integrations", {
    ...
    const data = await json(r);
    assert.ok((data.error as string).includes("name"), `expected name error, got: ${data.error}`);
  });

  await runTest("POST /sdk-integrations — rejects invalid environment (400)", async () => {
    const r = await authedFetch("POST", "/sdk-integrations", {
      name: "Bad Env",
  ...
  await runTest("GET /sdk-integrations — lists the newly created integration", async () => {
    const r = await authedFetch("GET", "/sdk-integrations");
    const data = await json(r);
    ...
    assert.ok(found.sdkKeyMasked.includes("..."), "SDK key should be masked in list");
  });

  await runTest("GET /sdk-integrations/:id — returns single integration", async () => {
  ...
  await runTest("GET /sdk-integrations/nonexistent — returns 404", async () => {
    const r = await authedFetch("GET", "/sdk-integrations/000000000000000000000001");
    assert.equal(r.status, 404);
  });

  await runTest("PATCH /sdk-integrations/:id — updates name and description", async () => {
    const r = await authedFetch("PATCH", `/sdk-integrations/${integrationId}`, {
...
async function testSdkKeyAuth() {
  ...
  await runTest("Engagement runtime — valid SDK key (development) → 200", async () => {
    ...
    const data = await json(r);
    assert.ok(Array.isArray((data.data as any)?.experiences), "should return experiences array");
  });
  ...
  await runTest("Engagement runtime — no key → 401", async () => {
    const r = await fetch(`${BASE}/engagement/runtime`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: "u1", sessionId: "s1", properties: {}, context: {} }),
    });
  ...
  await runTest("Engagement runtime — wrong origin (production) → 403", async () => {
...
async function testLifecycleActions() {
  ...
  await runTest("POST /:id/disable — disables integration", async () => {
    const r = await authedFetch("POST", `/sdk-integrations/${integrationId}/disable`);
    assert.equal(r.status, 200);
    const data = await json(r);
    assert.equal((data.data as any)?.integration?.status, "disabled");
  });

  await runTest("Engagement runtime — disabled integration key → 403", async () => {
    const r = await sdkFetch("POST", "/engagement/runtime", {
      userId: "u1",
      sessionId: "s1",
      properties: {},
  ...
  await runTest("POST /:id/enable — re-enables integration", async () => {
    const r = await authedFetch("POST", `/sdk-integrations/${integrationId}/enable`);
    assert.equal(r.status, 200);
    const data = await json(r);
    const status = (data.data as any)?.integration?.status;
    assert.ok(status === "connected" || status === "pending", `unexpected status: ${status}`);
  });

  await runTest("POST /:id/regenerate-key — rotates key, returns new raw key", async () => {
    const r = await authedFetch("POST", `/sdk-integrations/${integrationId}/regenerate-key`);
    assert.equal(r.status, 200);
    const data = await json(r);
    const newKey = (data.data as any)?.sdkKey as string;
    assert.ok(newKey?.startsWith("sdk_"), `new key should start with sdk_, got: ${newKey}`);
...
async function testEngagementSDKEvents() {
  ...
  await runTest("POST /engagement/track — guide_shown event → 200", async () => {
  ...
  await runTest("POST /engagement/track — guide_completed event → 200", async () => {
    const r = await sdkFetch("POST", "/engagement/track", {
  ...
  await runTest("POST /engagement/track — guide_dismissed event → 200", async () => {
    const r = await sdkFetch("POST", "/engagement/track", {
  ...
  await runTest("POST /engagement/track — survey_started event → 200", async () => {
    const r = await sdkFetch("POST", "/engagement/track", {
...
async function testConnectionTracking() {
  ...
  await runTest("Integration reflects connected status after runtime call", async () => {
...
async function testSurveyRuntimeTriggering() {
  ...
  await runTest("Runtime on /dashboard path — evaluates targeting rules", async () => {
    const r = await sdkFetch("POST", "/engagement/runtime", {
    ...
    console.log(`      → returned ${experiences.length} experience(s)`);
  });

  await runTest("Runtime on /billing path — evaluates NPS survey targeting", async () => {
    const r = await sdkFetch("POST", "/engagement/runtime", {
    ...
    const data = await json(r);
    const experiences = (data.data as any)?.experiences as any[];
    console.log(`      → returned ${experiences.length} experience(s) on /billing`);
  });

  await runTest("Runtime with event context — subscription_purchased triggers NPS survey", async () => {
    const r = await sdkFetch("POST", "/engagement/runtime", {
      ...
      context: {
        ...
        path: "/billing",
        title: "Billing",
        events: [{ eventName: "subscription_purchased", timestamp: new Date().toISOString() }],
      },
    });
    assert.equal(r.status, 200);
    const data = await json(r);
    const experiences = (data.data as any)?.experiences as any[];
    console.log(`      → returned ${experiences.length} experience(s) on event trigger`);
  });
}
...
async function testCleanup() {
  ...
  await runTest("DELETE /sdk-integrations/:id — removes integration", async () => {
    const r = await authedFetch("DELETE", `/sdk-integrations/${integrationId}`);
    assert.equal(r.status, 200);
    const data = await json(r);
  ...
  await runTest("SDK key after delete — returns 401 (key no longer valid)", async () => {
    const r = await sdkFetch("POST", "/engagement/runtime", {
      userId: "u1",
      sessionId: "s1",
      properties: {},
    ...
    assert.equal(r.status, 401, "deleted integration's key should be invalid");
  });
}
...
async function main() {
  ...
  console.log("\x1b[1m\x1b[35m  SDK Integration Management — API Integration Tests\x1b[0m");
