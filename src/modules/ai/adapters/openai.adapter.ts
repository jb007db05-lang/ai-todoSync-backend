...
export class OpenAIAdapter implements ILlmProviderAdapter {
  public async execute(
    ...
    if (!apiKey) {
      throw new Error(
        "OpenAI API key is missing. Please configure your API key in Settings.",
      );
    }

    const model = options.modelId || "gpt-4o";
    let url = options.baseUrl?.trim() || "https://api.openai.com/v1";
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `OpenAI execution failed (${response.status}): ${errText || response.statusText}`,
      );
    }
