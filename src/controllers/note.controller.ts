...
class NoteController {
  public createNote = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const note = await noteService.createNote(user._id.toString(), projectId, req.body as NotePayload);
  ...
  public getProjectNotes = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      const user = req.user;
  ...
  public updateNote = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const noteId = getRouteParam(req.params.id);
      const note = await noteService.updateNote(user._id.toString(), noteId, req.body as NotePayload);
  ...
  public deleteNote = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const noteId = getRouteParam(req.params.id);
      const deletedNoteId = await noteService.deleteNote(user._id.toString(), noteId);
